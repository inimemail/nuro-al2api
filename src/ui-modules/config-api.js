...
async function _handleUpdateConfig(req, res, currentConfig, body) {
    try {
        ...
        // Handle system prompt update only when an explicit prompt file path is configured.
        if (newConfig.systemPrompt !== undefined) {
            const promptPath = currentConfig.SYSTEM_PROMPT_FILE_PATH;
            if (!promptPath) {
                currentConfig.SYSTEM_PROMPT_CONTENT = null;
            } else {
                try {
                    ...
                    currentConfig.SYSTEM_PROMPT_CONTENT = newConfig.systemPrompt || null;
