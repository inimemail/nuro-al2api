...
/**
 * 代理 Agent 缓存，按 proxyUrl 复用 Agent 实例，避免每次请求都创建新连接池
 * @type {Map<string, Object>}
 */
const proxyAgentCache = new Map();

/**
 * 解析代理URL并返回相应的代理配置（带缓存）
...
export function parseProxyUrl(proxyUrl) {
    ...
    // 检查缓存
    if (proxyAgentCache.has(trimmedUrl)) {
        return proxyAgentCache.get(trimmedUrl);
    }
    ...
    try {
        ...
        if (protocol === 'socks5:' || protocol === 'socks4:' || protocol === 'socks:') {
            // SOCKS 代理 - 为 HTTP 和 HTTPS 分别创建独立实例，避免 TLS 状态冲突
            result = {
                httpAgent: new SocksProxyAgent(trimmedUrl),
                httpsAgent: new SocksProxyAgent(trimmedUrl),
                proxyType: 'socks'
            };
        ...
        // 存入缓存
        proxyAgentCache.set(trimmedUrl, result);
        return result;
    } catch (error) {
