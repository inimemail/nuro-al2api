...
export class OpenAIResponsesApiService {
    ...
    async callApi(endpoint, body, isRetry = false, retryCount = 0) {
        ...
        try {
            ...
            if (status === 401 || status === 403) {
                logger.error(`[OpenAI Responses API] Received ${status}. API Key might be invalid or expired.`);
            ...
            if (status === 429) {
                ...
                if (retryAfter !== null) {
                    logger.warn(`[OpenAI Responses API] Received 429 with Retry-After: ${retryAfter}ms. Throwing to upper layer.`);
                    error.shouldSwitchCredential = true;
                    error.skipErrorCount = true;
                    throw error;
                }
                if (retryCount < maxRetries) {
                    ...
                    logger.info(`[OpenAI Responses API] Received 429. Retrying in ${delay}ms... (attempt ${retryCount + 1}/${maxRetries})`);
            ...
            if (status >= 500 && status < 600 && retryCount < maxRetries) {
                const delay = baseDelay * Math.pow(2, retryCount);
                logger.info(`[OpenAI Responses API] Received ${status} server error. Retrying in ${delay}ms... (attempt ${retryCount + 1}/${maxRetries})`);
                await new Promise(resolve => setTimeout(resolve, delay));
                return this.callApi(endpoint, body, isRetry, retryCount + 1);
            }
            ...
            if (isNetworkError && retryCount < maxRetries) {
                ...
                logger.info(`[OpenAI Responses API] Network error (${errorIdentifier}). Retrying in ${delay}ms... (attempt ${retryCount + 1}/${maxRetries})`);
                await new Promise(resolve => setTimeout(resolve, delay));
                return this.callApi(endpoint, body, isRetry, retryCount + 1);
            }
    ...
    async *streamApi(endpoint, body, isRetry = false, retryCount = 0) {
        ...
        try {
            ...
            if (status === 401 || status === 403) {
                logger.error(`[OpenAI Responses API] Received ${status} during stream. API Key might be invalid or expired.`);
                error.shouldSwitchCredential = true;
                error.skipErrorCount = true;
                throw error;
            }
            ...
            if (status === 429) {
                const retryAfter = getRetryAfterMs(error);
                if (retryAfter !== null) {
                    logger.warn(`[OpenAI Responses API] Received 429 with Retry-After: ${retryAfter}ms during stream. Throwing to upper layer.`);
                    error.shouldSwitchCredential = true;
                    error.skipErrorCount = true;
                    throw error;
                }
                if (retryCount < maxRetries) {
                    const delay = baseDelay * Math.pow(2, retryCount);
                    logger.info(`[OpenAI Responses API] Received 429 during stream. Retrying in ${delay}ms... (attempt ${retryCount + 1}/${maxRetries})`);
                    await new Promise(resolve => setTimeout(resolve, delay));
            ...
            if (status >= 500 && status < 600 && retryCount < maxRetries) {
                const delay = baseDelay * Math.pow(2, retryCount);
                logger.info(`[OpenAI Responses API] Received ${status} server error during stream. Retrying in ${delay}ms... (attempt ${retryCount + 1}/${maxRetries})`);
                await new Promise(resolve => setTimeout(resolve, delay));
                yield* this.streamApi(endpoint, body, isRetry, retryCount + 1);
                return;
            }
            ...
            if (isNetworkError && retryCount < maxRetries) {
                const delay = baseDelay * Math.pow(2, retryCount);
                const errorIdentifier = errorCode || errorMessage.substring(0, 50);
                logger.info(`[OpenAI Responses API] Network error (${errorIdentifier}) during stream. Retrying in ${delay}ms... (attempt ${retryCount + 1}/${maxRetries})`);
                await new Promise(resolve => setTimeout(resolve, delay));
                yield* this.streamApi(endpoint, body, isRetry, retryCount + 1);
                return;
            }
