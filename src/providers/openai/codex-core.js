...
export class CodexApiService {
     ...
     * ?? Codex responses ???
     */
    async prepareRequestBody(model, requestBody, stream = true) {
        const normalizedModel = String(model || '').trim();
        const isFastModel = normalizedModel.endsWith('-fast');
        const upstreamModel = isFastModel ? normalizedModel.slice(0, -5) : normalizedModel;
        const isImageModel = IMAGE_MODELS.has(upstreamModel);
        const effectiveUpstreamModel = upstreamModel;
        const defaultServiceTier = isFastModel ? 'priority' : 'auto';
        const defaultReasoningEffort = isFastModel ? 'low' : 'medium';
        const sessionId = requestBody.session_id || requestBody.sessionId || requestBody.conversation_id || requestBody.conversationId || 'default';
        const cleanedBody = { ...requestBody };

        delete cleanedBody._monitorRequestId;
        delete cleanedBody._requestBaseUrl;
        delete cleanedBody.session_id;
        delete cleanedBody.sessionId;
        delete cleanedBody.conversation_id;
        delete cleanedBody.conversationId;
