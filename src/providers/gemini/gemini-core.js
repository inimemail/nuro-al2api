...
export class GeminiApiService {
    constructor(config) {
        ...
        const clientConfig = resolveGeminiOAuthClientConfig(config);
        const oauth2Options = {
    ...
    async * generateContentStream(model, requestBody) {
        ...
        for await (const chunk of stream) {
            const geminiResponse = toGeminiApiResponse(chunk.response);
            if (geminiResponse !== null && geminiResponse !== undefined) {
